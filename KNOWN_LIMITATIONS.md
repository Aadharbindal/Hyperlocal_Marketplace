...
| Warranty claims | **implemented** | every quote has carried a warranty since M4 and nothing could be claimed against it. A claim now goes to the professional with a 48-hour clock, escalates to support if ignored, and books a free return visit when accepted. The reward for a *referral* is still credited by support rather than automatically | — | — |
...
| Mobile component tests | **started** | 18 tests covering the shared UI primitives and the warranty card. Far from coverage, but the harness exists and the next test is cheap | broader component and screen coverage | ongoing |
