...
export function financeService(d: FinanceDeps) {
  ...
  function payoutAccountView(a: PayoutAccountRecord): PayoutAccountView {
    return {
      id: a.id,
      method: a.method,
      accountHolderName: a.account_holder_name,
      // Only the last four digits were ever stored, so there is nothing here left to mask.
      masked: a.method === 'BANK_ACCOUNT' ? `****${a.account_last4 ?? ''}` : maskVpa(a.vpa ?? ''),
      ifsc: a.ifsc,
      status: a.status,
      rejectionReason: a.rejection_reason,
      readyForPayouts: a.status === 'VERIFIED' && !!a.provider_fund_account_id,
      createdAt: a.created_at.toISOString(),
    };
  }

  /**
   * What is owed is recorded whether or not we know where to send it - a debt the payee cannot
   * see is worse than one that is waiting. If there is nowhere to pay, the settlement is parked
   * and the payee is asked for their details; adding them releases it.
   */
  async function parkIfUnpayable(settlement: SettlementRecord): Promise<SettlementRecord> {
    if (await payoutHandle(settlement.payee_id)) return settlement;
    const parked = await store.finance.updateSettlement(settlement.id, {
      status: 'ON_HOLD',
      failure_reason: 'NO_PAYOUT_ACCOUNT',
    });
    await notify(
      settlement.payee_id,
      'payout.account_missing',
      'Add your bank details',
      'Your earnings for this job are ready, but we do not know where to send them yet. Add your bank or UPI details to get paid.',
      settlement.job_id,
    );
    return parked;
  }

  /** The handle money can be sent to, or null when the payee has not said where to send it. */
  async function payoutHandle(userId: string): Promise<string | null> {
    const a = await store.finance.getPayoutAccount(userId);
    return a && a.status === 'VERIFIED' ? a.provider_fund_account_id : null;
  }
  ...
  async function bookingPayment(jobId: string): Promise<PaymentRecord | null> {
    ...
    return (
      payments.find(
        (p) => p.purpose === 'BOOKING' && p.status !== 'FAILED' && p.status !== 'RELEASED',
      ) ?? null
    );
  }
  ...
  async function capturedAllocation(jobId: string) {
    ...
    const captured = entries.filter(
      (e) => e.reference_type === 'booking_quote' || e.reference_type === 'cancellation',
    );
    const by = (type: string) =>
      captured.filter((e) => e.entry_type === type).reduce((t, e) => t + Number(e.amount_paise), 0);
    return {
  ...
  return {
    ...
    async captureForJob(job: JobRecord, ctx: TransitionContext) {
      ...
      const problem = checkCanCapture(job, payment, {
      ...
      await adapters.payment.capture({
      ...
      await postBatch(
        ...
        {
          idempotencyKey: `cap_${p.id}`,
          jobId: job.id,
          paymentId: p.id,
          referenceType: 'booking_quote',
      ...
      await notify(
        ...
        'payment.captured',
        'Payment received',
        'The customer approved the work. Your payout is being prepared.',
        job.id,
      );
      adapters.analytics.track('payment_captured', {
    ...
    async captureMaterialOrder(order: MaterialOrderRecord) {
      ...
      await postBatch(
        ...
        {
          idempotencyKey: `mat_cap_${order.id}`,
          jobId: order.job_id,
      ...
      adapters.analytics.track('material_captured', {
    ...
    // ------------------------------------------------------------ payout account
    /**
     * Where a payee's money goes. The bank details are handed straight to the payment provider
     * and never stored here - we keep the last four digits to recognise the account by, and the
     * handle the provider gives back, which is the only thing a payout can actually be sent to.
     */
    async setPayoutAccount(
      userId: string,
      input: PayoutAccountBody,
      requestId: string | null,
    ): Promise<PayoutAccountView> {
      const user = await store.users.findById(userId);
      if (!user) throw notFound('user');

      const problem = checkPayoutAccount(input);
      if (problem) blocked(problem);

      const previous = await store.finance.getPayoutAccount(userId);

      // Registered with the provider first, on purpose: if the rail refuses the details there
      // must not be a row left behind claiming money can be sent.
      let registration: { contactId: string; fundAccountId: string };
      try {
        registration = await adapters.payment.registerPayee(
        ...
        adapters.monitoring.captureError(e, { scope: 'payout_account_register' });
        blocked('PAYOUT_PROVIDER_REFUSED');
      }

      const record = await store.finance.createPayoutAccount({
        user_id: userId,
        method: input.method,
        account_holder_name: input.accountHolderName,
        account_last4: input.method === 'BANK_ACCOUNT' ? input.accountNumber.slice(-4) : null,
        ifsc: input.method === 'BANK_ACCOUNT' ? input.ifsc.toUpperCase() : null,
        vpa: input.method === 'UPI' ? input.vpa : null,
        provider_contact_id: registration.contactId,
        provider_fund_account_id: registration.fundAccountId,
        // The provider accepting the account is what "verified" means here: the account exists
        // and is payable. It is *not* proof the name on it is the payee's - that needs a
        // penny-drop or a name-match check, which is a paid API and is not wired yet
        // (KNOWN_LIMITATIONS).
        status: 'VERIFIED',
      ...
      // Changing where money goes is exactly the kind of change that has to be explainable
      // afterwards, so it is written to the audit log - without the account details.
      await store.audit.append({
        actor_user_id: userId,
        actor_role: null,
        action: previous ? 'payout_account.replaced' : 'payout_account.added',
        entity_type: 'payout_account',
        entity_id: record.id,
        reason: null,
        before: previous ? { id: previous.id, method: previous.method } : null,
        after: { method: record.method },
        ip: null,
        request_id: requestId,
      });

      // Anything that was parked for want of an account can now be tried again.
      for (const s of await store.finance.listSettlementsForPayee(userId, 50)) {
        if (s.status === 'ON_HOLD' && s.failure_reason === 'NO_PAYOUT_ACCOUNT') {
          await store.finance.updateSettlement(s.id, { status: 'PENDING', failure_reason: null });
        }
      }

      adapters.analytics.track('payout_account_set', { userId });
      return payoutAccountView(record);
    },

    async getPayoutAccount(userId: string): Promise<PayoutAccountView | null> {
      const record = await store.finance.getPayoutAccount(userId);
      return record ? payoutAccountView(record) : null;
    },
    ...
    async runSettlements(limit: number, actorId: string | null) {
      ...
      for (const s of due) {
        ...
        const payeeProfile =
          s.payee_role === 'VENDOR'
            ? await store.users.getVendorProfile(s.payee_id)
            : await store.users.getProviderProfile(s.payee_id);
        const problem = job
          ? checkCanSettle({
              ...
              hasPayoutAccount: true, // checked on its own below, so it can be retried rather than parked
            })
          : 'JOB_NOT_COMPLETE';
        ...
        if (problem) {
          await store.finance.updateSettlement(s.id, {
            status: 'ON_HOLD',
        ...
        // The payout rail pays an account it knows about, never a user id of ours.
        const handle = await payoutHandle(s.payee_id);
        if (!handle) {
          await parkIfUnpayable(s);
          results.push({ id: s.id, status: 'ON_HOLD', reason: 'NO_PAYOUT_ACCOUNT' });
          continue;
        }

        const initiated = await store.finance.updateSettlement(s.id, {
          status: 'INITIATED',
        ...
        if (!payout.ok) {
          ...
          await store.finance.updateSettlement(s.id, {
            status,
            failure_reason: payout.failureReason ?? 'payout_failed',
          });
        ...
        await store.finance.updateSettlement(s.id, {
          status: 'PAID',
          paid_at: new Date(),
          provider_transfer_id: payout.transferId,
        });
        await postBatch(
          buildSettlementLines({
            amountPaise: Number(s.amount_paise),
        ...
        await notify(
          ...
          'payout.sent',
          'Payout sent',
          'Your earnings for this job are on the way to your account.',
          s.job_id,
        );
    ...
    async cancellationQuote(job: JobRecord) {
      ...
      const charge = quote
        ? customerCancellationCharge(stage, {
     ...
     */
    async cancelWithMoney(
      job: JobRecord,
      userId: string,
      reason: string,
      byRole: 'CUSTOMER' | 'PROVIDER',
      ...
      const cancelled = await jobs.transition(
        job,
        byRole === 'CUSTOMER' ? 'CANCELLED_BY_CUSTOMER' : 'CANCELLED_BY_PROVIDER',
        ctx,
        {
          reason,
            ...
            [
              {
                ...
                amountPaise: charge,
                accountUserId: job.customer_id,
                note: 'Cancellation charge',
              },
              {
                entryType: 'PROVIDER_PAYABLE',
                amountPaise: -charge,
                accountUserId: quote.provider_id,
              },
            ],
            {
              idempotencyKey: `cancel_${job.id}`,
              jobId: job.id,
              paymentId: payment.id,
      ...
      adapters.analytics.track('job_cancelled', {
        userId,
        jobId: job.id,
        byRole,
        chargePaise: charge,
    ...
    async refund(
      ...
      amountPaise: number,
      reason: string,
      opts: { requestedBy: string | null; disputeId?: string | null; key?: string } = {
        requestedBy: null,
      },
    ) {
      const payment = await bookingPayment(job.id);
      ...
      if (already + amountPaise > captured.totalPaise) {
        blocked('REFUND_EXCEEDS_CAPTURE', {
          capturedPaise: captured.totalPaise,
      ...
      const done = await store.finance.updateRefund(record.id, {
        status: 'COMPLETED',
        provider_refund_id: gateway.providerRefundId,
      });
      ...
      adapters.analytics.track('refund_issued', {
        userId: job.customer_id,
        jobId: job.id,
        amountPaise,
      });
    ...
    async raiseDispute(
      ...
      const providerIds = assignment
        ? [assignment.provider_id, assignment.technician_id].filter((x): x is string => !!x)
        : [];
      ...
      const againstId =
        job.customer_id === userId ? (assignment?.provider_id ?? null) : job.customer_id;
      ...
      for (const mediaId of input.mediaIds ?? []) {
        await store.finance.addEvidence({
      ...
      if (payment && (payment.status === 'CAPTURED' || payment.status === 'AUTHORIZED')) {
        ...
        if (quote && payment.status === 'CAPTURED') {
          await postBatch(
            buildDisputeHoldLines({
              amountPaise: Number(quote.provider_payable_paise),
              providerId: quote.provider_id,
              jobRef: job.id,
            }),
            {
              idempotencyKey: `hold_${dispute.id}`,
              jobId: job.id,
              paymentId: payment.id,
      ...
      for (const s of (await store.finance.listSettlementsForPayee(againstId ?? userId, 50)).filter(
        (x) => x.job_id === job.id && x.status === 'PENDING',
      )) {
        await store.finance.updateSettlement(s.id, {
          status: 'ON_HOLD',
          failure_reason: 'dispute_open',
        });
      }
      ...
      if (job.status !== 'COMPLETED' && job.status !== 'SETTLED') {
        await jobs.transition(job, 'DISPUTED', ctx, {
          reason: 'Dispute raised',
          metadata: { disputeId: dispute.id, category: input.category },
        });
      }
      ...
      if (againstId)
        await notify(
          ...
          'Support will contact you. Please add anything that helps.',
          job.id,
        );
      adapters.analytics.track('dispute_raised', {
        userId,
        jobId: job.id,
        category: input.category,
      });
    ...
    async addDisputeEvidence(
      ...
      if (dispute.status === 'OPEN')
        await store.finance.updateDispute(dispute.id, { status: 'UNDER_REVIEW' });
      return added;
    },
    ...
    async resolveDispute(
      ...
      input: {
        ...
        reason: string;
        refundPaise?: number;
        strike?: { userId: string; severity: StrikeSeverity; reason: string };
      ...
      if (refundNeedsTwoPeople(refund)) {
        if (!input.secondApproverId)
          blocked('SECOND_APPROVER_REQUIRED', { thresholdPaise: 5_00_000 });
      ...
      if (refund > 0 && payment) {
        ...
        if (payment.status === 'DISPUTE_HOLD' && quote) {
          ...
          await postBatch(
            ...
            {
              idempotencyKey: `rel_${dispute.id}`,
              jobId: job.id,
              paymentId: payment.id,
              referenceType: 'dispute',
              referenceId: dispute.id,
      ...
      if (input.strike) {
        await this.addStrike(input.strike.userId, input.strike.severity, input.strike.reason, {
          jobId: job.id,
          disputeId: dispute.id,
          issuedBy: adminId,
        });
      ...
      if (job.status === 'DISPUTED') {
        const to: JobStatus =
          input.resolution === 'REWORK' || input.resolution === 'REPLACEMENT_PROVIDER'
            ? 'IN_PROGRESS'
            : input.resolution === 'FULL_REFUND'
              ? 'REFUNDED'
              : 'COMPLETED';
        await jobs.transition(job, to, ctx, {
      ...
      for (const s of (
        await store.finance.listSettlementsForPayee(dispute.against_user_id ?? adminId, 50)
      ).filter((x) => x.job_id === job.id && x.status === 'ON_HOLD')) {
        await store.finance.updateSettlement(s.id, {
          status: refund > 0 ? 'ON_HOLD' : 'PENDING',
          failure_reason: refund > 0 ? 'refunded' : null,
        });
      }
      ...
      adapters.analytics.track('dispute_resolved', {
        userId: adminId,
        jobId: job.id,
        resolution: input.resolution,
        refundPaise: refund,
    ...
    async addStrike(
      ...
      reason: string,
      opts: { jobId?: string | null; disputeId?: string | null; issuedBy: string | null },
    ) {
      ...
      if (profile) {
        ...
        const suspend = shouldSuspend(
          strikes.map((s) => ({ severity: s.severity, createdAt: s.created_at })),
        );
        ...
        if (suspend) {
          await notify(
            ...
            'Your account is paused while support reviews recent issues.',
            opts.jobId ?? '',
          );
        }
      }
    ...
    async review(job: JobRecord, userId: string, input: { rating: number; comment?: string }) {
      ...
      if (profile) {
        const next = nextRating(
          { ratingAvg: profile.rating_avg, ratingCount: profile.rating_count },
          input.rating,
        );
        await store.users.upsertProviderProfile({
          ...profile,
          rating_avg: next.ratingAvg,
    ...
    async earnings(userId: string) {
      ...
      const pending = settlements
        .filter((s) => s.status === 'PENDING' || s.status === 'INITIATED')
        .reduce((t, s) => t + Number(s.amount_paise), 0);
      const settled = settlements
        .filter((s) => s.status === 'PAID')
        .reduce((t, s) => t + Number(s.amount_paise), 0);
      const onHold = settlements
        .filter((s) => s.status === 'ON_HOLD' || s.status === 'FAILED')
        .reduce((t, s) => t + Number(s.amount_paise), 0);
      const account = await store.finance.getPayoutAccount(userId);
      // Parked for want of an account. Counted separately because it is not stuck on a dispute
      // or a failure - it is one form away from being paid.
      const awaitingAccount = settlements
        .filter((s) => s.status === 'ON_HOLD' && s.failure_reason === 'NO_PAYOUT_ACCOUNT')
        .reduce((t, s) => t + Number(s.amount_paise), 0);
      return {
        ...
        payoutAccount: account ? payoutAccountView(account) : null,
        awaitingPayoutAccountPaise: awaitingAccount,
    ...
    async jobMoney(job: JobRecord, viewerId: string) {
      if (job.customer_id !== viewerId) {
        const assignment = await store.negotiation.getActiveAssignment(job.id);
        const onJob = [assignment?.provider_id, assignment?.technician_id]
          .filter(Boolean)
          .includes(viewerId);
        if (!onJob) throw forbidden('not on this job');
      }
      ...
      return {
        jobId: job.id,
        authorizedPaise:
          booking && booking.status === 'AUTHORIZED' ? Number(booking.amount_paise) : 0,
        capturedPaise:
          booking &&
          ['CAPTURED', 'SETTLED', 'PARTIALLY_REFUNDED', 'DISPUTE_HOLD'].includes(booking.status)
            ? Number(booking.amount_paise)
            : 0,
        refundedPaise: refunds
          .filter((r) => r.status === 'COMPLETED')
          .reduce((t, r) => t + Number(r.amount_paise), 0),
        materialPaise: payments
          .filter((p) => p.purpose === 'MATERIAL' && p.status !== 'FAILED')
          .reduce((t, p) => t + Number(p.amount_paise), 0),
